...
var loader = function (ctx) {
	...
	var SDK = ctx.APP.SDK,
			...
			checkInit = function () {
				!ctx.APP.SDK.initialized && error("not_initialized");
			},
			...
			Validation = {
				ensureMeasurementType: function(value) {
					if (!ctx.APP.SDK.Validation.isMeasurementType(value)) {
						error("invalid_measurement_type", {type: value});
					}
					return value;
				},
				ensureMeasurementSource: function(value) {
					if (!ctx.APP.SDK.Validation.isMeasurementSource(value)) {
			...
			changeAgentStatus = function(id, status, cb) {
				if (!ctx.APP.SDK.Validation.isAgentStatus(status)) {
	...
	ctx.APP.SDK = {
		Agent: {
			...
			disable: function(id, cb) {
				changeAgentStatus(id, ctx.APP.SDK.AgentStatus.disable, cb);
			},
			/**
			 * Enable an agent
			 *
			...
			enable: function(id, cb) {
				changeAgentStatus(id, ctx.APP.SDK.AgentStatus.enable, cb);
			},
			/**
			...
			get: function(idOrAgentDisposition, cb) {
				var path = "agent/",
						drill = [];
				if (ctx.APP.SDK.Validation.isAgentDisposition(idOrAgentDisposition)) {
			 ...
			 */
			halt: function(id, cb) {
				changeAgentStatus(id, ctx.APP.SDK.AgentStatus.halt, cb);
			},
			/**
			...
			start: function(id, cb) {
				changeAgentStatus(id, ctx.APP.SDK.AgentStatus.start, cb);
			},
			/**
			 * Stop an agent
			 *
			 ...
			 */
			stop: function(id, cb) {
				changeAgentStatus(id, ctx.APP.SDK.AgentStatus.stop, cb);
			},
			Status: {
				/**
		...
		Aggregables: {
			...
			copyChannels: function(key, typefrom, from, typeto, to, cb) {
				if (!ctx.APP.SDK.Validation.isUUID(from)) {
					error("invalid_UUID", {"value": from});
				}
				if (!ctx.APP.SDK.Validation.isUUID(to)) {
		...
		Aggregation: {
			...
			get: function (entityType, entityId, cb) {
				...
				else if (ctx.APP.SDK.Validation.isEntityType(entityType)) {
		...
		AssetMeasurement: function(asset, channel, timestamp, value, type) {
			if (!ctx.APP.SDK.Validation.isUUID(asset)) {
				error("invalid_uuid", {value: asset});
			}
			if (!ctx.APP.SDK.Validation.isFloat(value)) {
			...
			this.value = value;
			this.type = type ? type : ctx.APP.SDK.MeasurementType.interval;
		},
		Csv: {
			/**
		...
		Label: {
			...
			changeBehavior: function(id, labelBehavior, cb) {
				if (!ctx.APP.SDK.Validation.isLabelBehavior(labelBehavior)) {
			...
			get: function(idOrBehavior, cb) {
				if ($.isFunction(idOrBehavior)) {
					...
					var drill = ctx.APP.SDK.Validation.isLabelBehavior(idOrBehavior) ? ["label"] : null;
		...
		Measurement: {
			...
			get: function(source, type, assetId, timespan, channel, limit, cb) {
				...
				(function(args) {
					...
					if (isUUID(source) || args.length == 2) {
						...
						source = ctx.APP.SDK.MeasurementSource.aggregated;
					}
			...
			last: function(id, type, n, cb) {
				doGet.call(this, "measurement/" + ctx.APP.SDK.MeasurementSource.raw + "/" + type + "/" + id + "/last/" + n, null, null, cb);
			},
			/**
		...
		Validation: {
			isAgentDisposition: function(value) {
				return isValid(value, ctx.APP.SDK.AgentDisposition);
			},
			isAgentStatus: function(value) {
				return isValid(value, ctx.APP.SDK.AgentStatus);
			},
			isEntityType: function(value) {
				return isValid(value, ctx.APP.SDK.EntityType);
			},
			isLabelBehavior: function(value) {
				return isValid(value, ctx.APP.SDK.LabelBehavior);
			},
			isMeasurementType: function(value) {
				return isValid(value, ctx.APP.SDK.MeasurementType);
			},
			isMeasurementSource: function(value) {
				return isValid(value, ctx.APP.SDK.MeasurementSource);
			},
		...
		init: function ($, config, cb) {
			if (!ctx.APP.SDK.initialized) {
				setConfig(config);
				ctx.APP.SDK.initialized = true;
			}
			else {
	...
	ctx.APP.SDK.Label.find = ctx.APP.SDK.Label.search;
	ctx.APP.SDK.Label.remove = ctx.APP.SDK.Label.delete;
	ctx.APP.SDK.Asset.search = ctx.APP.SDK.Asset.find;
	ctx.APP.SDK.Asset.delete = ctx.APP.SDK.Asset.remove;
	for (var j in ctx.APP.SDK) {
		addPaging(ctx.APP.SDK[j]);
	}
}
loader(window);
window.APP.SDK.cloneInto = function(ctx) {
	loader(ctx);
}
})();
